function GeneratorOptions(colorAssignment, spritesToUseForChambers) constructor {
	...
	self.closeWallCorners = false;	//	Wether corners on walls should be closed
...
function RandomDungonGenerator(options) constructor {
	...
	static generateDungeon = function() {
		...
		self.dungeonPreset.createNewDungeon(self.chamberPresets, self.options.amountOfChambersToPlace,self.options.minimumRandomOffsetBetweenPlacedChambers,self.options.maximumRandomOffsetBetweenPlacedChambers, self.options.surroundWithWalls, self.options.closeWallCorners);
